...
export default function MatchOverlay({ movie }: MatchOverlayProps) {
  const [showProviders, setShowProviders] = useState(false);
  // Controls the 1.65 second reveal delay
  const [isRevealed, setIsRevealed] = useState(false);

  const visibleProviders = movie.providers.slice(0, 2);
  const extraProviderCount = movie.providers.length - visibleProviders.length;
  ...
  useEffect(() => {
    // 1. Play the sound immediately on mount
    playSound();

    let interval: number;

    // 2. Set a timer to wait 1.65 seconds before revealing the contents and shooting confetti
    ...
    // Clean up both the timeout and the interval if the component unmounts early
    return () => {
            ...
            <div className={`grid gap-6 text-left lg:grid-cols-[1.2fr_0.8fr] transition-all duration-700 ease-out ${isRevealed ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
              <div className="glass-match-poster-col">
                ...
                <div className="space-y-3 p-6">
                  <p className="pickle-eyebrow !text-xs">Winning pick</p>
                  <h2 className="pickle-title text-3xl">{movie.title}</h2>
                  <p className="text-sm text-[var(--pickle-text-muted)]">
              ...
              <div className="glass-panel">
                <div className="glass-panel__inner flex h-full flex-col justify-between gap-6">
                  <div>
                    <p className="pickle-eyebrow !text-xs">Celebrate</p>
                    <h3 className="pickle-title mt-3 text-2xl"> A full room match!</h3>
                    <p className="pickle-subtitle mt-3 text-sm">
                      Every active participant swiped yes on this title, so the room has reached a unanimous Pickle.
                    </p>
                  </div>

                  <div className="space-y-3">
                    <p className="text-xs font-medium uppercase tracking-[0.2em] text-[var(--pickle-text-muted)]">
                      Streaming on
                    </p>
                    <div className="flex flex-wrap items-center gap-2">
                      {visibleProviders.map((provider) => (
                        <span key={provider.id} className="glass-chip glass-chip--selected inline-flex gap-2 !normal-case !tracking-normal">
                          {provider.logoPath ? (
                            <img src={provider.logoPath} alt={provider.name} className="h-5 w-5 rounded-full object-contain" />
                          ) : (
                            <span className="pickle-provider-logo inline-flex h-5 w-5 items-center justify-center rounded-full text-[0.6rem] font-semibold uppercase">
                      ...
                      {extraProviderCount > 0 ? (
                        <button
                          ...
                          className="glass-chip !normal-case !tracking-normal"
                        >
                          +{extraProviderCount} more
                        </button>
                      ) : null}
                    </div>
                    {showProviders ? (
                      <div ref={menuRef} className="glass-panel max-h-64 overflow-auto">
                        <div className="glass-panel__inner space-y-2 !py-3">
                          {movie.providers.map((provider) => (
                            <div key={provider.id} className="flex items-center gap-3 rounded-xl px-2 py-1">
                              {provider.logoPath ? (
                                <img src={provider.logoPath} alt={provider.name} className="h-6 w-6 rounded-full object-contain" />
                              ) : (
                                <span className="pickle-provider-logo inline-flex h-6 w-6 items-center justify-center rounded-full text-[0.6rem] font-semibold uppercase">
                                  {provider.name.slice(0, 2)}
                                </span>
                              )}
                  ...
                  <div className="space-y-2 text-sm">
                    <p className="text-xs font-medium uppercase tracking-[0.2em] text-[var(--pickle-text-muted)]">Movie details</p>
                    <p><span className="font-medium">Title:</span> {movie.title}</p>
                    <p><span className="font-medium">Type:</span> {movie.type}</p>
